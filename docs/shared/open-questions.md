...
| Client-side "Connect wallet" + real `sendPayment()` (ADR-009) builds and typechecks, but the actual login/send click-through was never verified end to end, no browser automation was available in this session | Intent/Design | Confirming the real send actually completes in production | Test the flow yourself at mova-rails.vercel.app/pay; if the login modal or send fails, check Dashboard -> Build -> Domains includes the production domain |
